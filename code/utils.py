...
def preprocess_data(data_path, preprocess_data_path, train=True):
    if train:
        ...
        np.save(os.path.join(preprocess_data_path, 'yelp_id_to_word.npy'),np.asarray(id_to_word))
        np.save(os.path.join(preprocess_data_path, 'yelp_word_to_id.npy'),np.asarray(word_to_id))
        ...
        if not os.path.exists(os.path.join(preprocess_data_path, 'yelp_word_to_id.npy')):
            raise FileNotFoundError('Dictionary not found, please process training set first')

        word_to_id = np.load(os.path.join(preprocess_data_path, 'yelp_word_to_id.npy')).item()
